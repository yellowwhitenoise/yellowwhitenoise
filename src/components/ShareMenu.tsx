...
function ShareIcon({ name }: { name: ShareIconName }) {
  if (name === "link") {
    // Chain-link glyph (SVG Repo), monochrome to match, slightly
    // smaller than the other tiles by request.
    return (
      <svg viewBox="0 0 512 512" className="h-6 w-6" fill="currentColor" aria-hidden>
        ...
        <path d="M160.219,351.781c12.234,12.25,32.063,12.25,44.281,0L351.781,204.5c12.219-12.219,12.25-32.031,0-44.281c-12.234-12.219-32.063-12.219-44.281,0L160.219,307.531C148,319.75,148,339.563,160.219,351.781z" />
      </svg>
    );
  }
  if (name === "x") {
  ...
  if (name === "facebook") {
    // Facebook "f" glyph (SVG Repo), monochrome to match, slightly
    // smaller than the other tiles by request.
    return (
      <svg viewBox="0 0 512 512" className="h-6 w-6" fill="currentColor" aria-hidden>
  ...
  if (name === "linkedin") {
    // LinkedIn glyph (SVG Repo), monochrome to match. Artwork coordinates
    // are offset, so the group is translated into the 20x20 viewBox.
    return (
  ...
  if (name === "messages") {
    // Message bubble glyph (SVG Repo), stroke adapted to match.
    return (
  ...
  if (name === "telegram") {
    ...
    // rendered one step larger by request.
    return (
      <svg viewBox="0 0 192 192" className="h-8 w-8" fill="none" stroke="currentColor" strokeWidth="12" aria-hidden>
